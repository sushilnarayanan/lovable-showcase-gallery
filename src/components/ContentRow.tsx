...
const ContentRow = ({
  ...
  const scroll = (direction: 'left' | 'right') => {
    if (rowRef.current) {
      ...
      const scrollAmount = direction === 'left' ? current.scrollLeft - current.clientWidth * 0.75 : current.scrollLeft + current.clientWidth * 0.75;
      current.scrollTo({
        left: scrollAmount,
        behavior: 'smooth'
      });
  ...
  return <div className="netflix-row mb-8"> {/* Consistent bottom margin */}
      ...
      <div className="group relative px-0">
        ...
        {canScrollRight && <button className="absolute right-0 top-0 bottom-0 z-40 bg-black/80 w-12 h-full flex items-center justify-center transition-opacity" onClick={() => scroll('right')} aria-label="Scroll right">
